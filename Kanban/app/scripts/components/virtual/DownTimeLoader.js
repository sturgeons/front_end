...
class Comp extends Component {
    componentWillReceiveProps(nextProps) {
        ...
        fetch(PREFIX + `STdowntimeService/find?LINE=${line}&OP=${value}`).then((resp) => {
            return resp.json();
        }).then((json) => {
            nextProps.onChangeValue({ downtime: json });
        }).catch(() => {
            // do something
        }).then(() => {
...
const mapDispatchToProps = (dispatch, ownProps) => ({
    onChangeValue: (value) => { dispatch(changeValue("downtime", value)) },
