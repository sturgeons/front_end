...
class Comp extends Component {
    componentWillReceiveProps(nextProps) {
        ...
        fetch(PREFIX + `STlastpartnoService/find?LINE=${line}&OP=${value}`).then((resp) => {
            return resp.json();
        }).then((json) => {
            nextProps.onChangeValue({lastpart: json.length ? json[0] : {}});
        }).catch(() => {
            // do something
        }).then(() => {
...
const mapDispatchToProps = (dispatch, ownProps) => ({
    onChangeValue: (value) => { dispatch(changeValue("lastpart", value)) },
